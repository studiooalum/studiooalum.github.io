...
- 루트의 [_redirects](/workspaces/studiooalum.github.io/_redirects) 로 www 호스트와 주요 .html 경로를 clean URL 로 정규화
...
이 저장소에는 Pages용 [_redirects](/workspaces/studiooalum.github.io/_redirects) 가 추가되어 있다. 다만 Cloudflare Dashboard 에서 Redirect Rule 을 별도로 운영 중이라면 우선순위가 더 높은 한 곳만 기준으로 유지하는 편이 안전하다.
