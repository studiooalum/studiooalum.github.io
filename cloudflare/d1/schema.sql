...
CREATE TABLE IF NOT EXISTS repair_requests (
  ...
  archive_consent_status TEXT NOT NULL DEFAULT 'unrecorded' CHECK(archive_consent_status IN ('agreed', 'declined', 'unrecorded')),
  marketing_opt_in INTEGER NOT NULL DEFAULT 0,
  ...
  bank_account TEXT NOT NULL DEFAULT '국민 한아름 218301-04-144506',
...
WITH template_seed(
  ...
  VALUES
    ...
    ('repair.application_submitted','email','repair','수선 접수 완료','수선 신청 직후 고객에게 보내는 상세 안내입니다.','접수 완료','[Studio OALUM] 수선 접수가 완료되었습니다','안녕하세요, {{customer_name}}님.' || char(10) || char(10) || 'Studio OALUM에 {{product_name}} 수선을 신청해주셔서 감사합니다.' || char(10) || '수선 접수 조회번호: {{repair_number}}' || char(10) || '제품 보내실 곳: {{studio_address}}' || char(10) || char(10) || '신청 내용과 이후 안내는 아래 수선 접수 페이지에서 확인해주세요.' || char(10) || '{{repair_ticket_url}}' || char(10) || char(10) || '실제 수선을 시작할 때 수선 티켓 번호가 발급됩니다.','["customer_name","product_name","repair_number","studio_address","repair_url","repair_ticket_url"]','["customer_name","product_name","repair_number","studio_address","repair_ticket_url"]',0,1),
    ('repair.application_submitted_admin','email','repair','새 수선 접수 · 관리자','고객이 수선 신청을 완료하면 관리자에게 보내는 필수 알림입니다.','접수 완료','[Repair] 새 수선 접수 · {{customer_name}}','새 수선 접수가 완료되었습니다. 고객 {{customer_name}}, 이메일 {{customer_email}}, 연락처 {{customer_phone}}, 제품 {{product_name}}, 고객 발송지 {{shipping_address}}. 수선 요청: {{repair_request}}. 관리자 확인: {{repair_admin_url}}','["customer_name","customer_email","customer_phone","product_name","repair_request","shipping_address","repair_ticket_url","repair_admin_url"]','["customer_name","customer_email","customer_phone","product_name","repair_request","repair_admin_url"]',0,1),
    ('repair.application_submitted','sms','repair','수선 접수 완료','국내 고객에게 보내는 수선 접수 문자입니다.','접수 완료','','[Studio OALUM 수선 안내]' || char(10) || '{{customer_name}}님, 수선 신청을 정상적으로 접수했습니다.' || char(10) || '조회번호: {{repair_number}}' || char(10) || '제품 발송 주소와 이후 안내: {{repair_ticket_url}}' || char(10) || '수선을 시작할 때 티켓 번호가 발급됩니다.','["customer_name","repair_number","repair_ticket_url"]','["customer_name","repair_number","repair_ticket_url"]',2000,1),
    ('repair.received','email','repair','수선 제품 수신 완료','수선 제품 도착 및 예상 가격 안내입니다.','제품 수신','[Studio OALUM] 수선 제품 도착 및 예상 가격 안내','안녕하세요, {{customer_name}}님.' || char(10) || char(10) || '보내주신 {{product_name}} 수선 제품을 잘 받았습니다.' || char(10) || '예상 가격: {{quote_amount}}' || char(10) || char(10) || '위 금액은 실제 작업 범위에 따라 달라질 수 있습니다.' || char(10) || '진행 내용 확인과 문의: {{repair_ticket_url}}','["customer_name","product_name","repair_number","quote_amount","repair_ticket_url"]','["customer_name","product_name","quote_amount","repair_ticket_url"]',0,1),
    ('repair.received','sms','repair','수선 제품 수신 완료','국내 고객 제품 수신 및 예상 가격 문자입니다.','제품 수신','','[Studio OALUM 수선 안내]' || char(10) || '{{customer_name}}님, 보내주신 수선 제품을 잘 받았습니다.' || char(10) || '예상 가격: {{quote_amount}}' || char(10) || '확인 및 문의: {{repair_ticket_url}}','["customer_name","quote_amount","repair_ticket_url"]','["customer_name","quote_amount","repair_ticket_url"]',2000,1),
    ('repair.repair_completed_quote_ready','email','repair','수선 완료 및 가격 안내','수선 완료 후 최종 가격 안내입니다.','수선 완료','[Studio OALUM] 수선 완료 및 결제 안내 · {{repair_number}}','안녕하세요, {{customer_name}}님.' || char(10) || char(10) || '{{product_name}} 수선 작업을 정성껏 마쳤습니다.' || char(10) || '접수 번호: {{repair_number}}' || char(10) || '최종 금액: {{final_amount}}' || char(10) || char(10) || '완료 사진과 결제 방법은 아래 Repair Ticket에서 확인해주세요.' || char(10) || '결제가 확인되면 제품을 안전하게 포장해 발송하겠습니다.' || char(10) || '{{repair_ticket_url}}' || char(10) || char(10) || '감사합니다.' || char(10) || 'Studio OALUM','["customer_name","product_name","repair_number","final_amount","repair_ticket_url"]','["customer_name","product_name","final_amount","repair_ticket_url"]',0,1),
    ('repair.repair_completed_quote_ready','sms','repair','수선 완료 및 가격 안내','국내 고객 수선 완료 문자입니다.','수선 완료','','[Studio OALUM 수선 안내]' || char(10) || '{{customer_name}}님, 수선 작업을 완료했습니다.' || char(10) || char(10) || '최종 금액: {{final_amount}}' || char(10) || '완료 사진과 결제 안내는 아래 수선 티켓에서 확인해주세요.' || char(10) || '{{repair_ticket_url}}','["customer_name","final_amount","repair_ticket_url"]','["customer_name","final_amount","repair_ticket_url"]',2000,1),
    ('repair.payment_confirmed_shipping_started','email','repair','입금 확인 및 배송 시작','입금 확인과 배송을 한 번에 안내합니다.','입금 확인 및 배송','[Studio OALUM] 입금 확인 및 발송 안내 · {{repair_number}}','안녕하세요, {{customer_name}}님.' || char(10) || char(10) || '입금을 확인했으며 {{product_name}} 수선 제품을 발송했습니다.' || char(10) || '접수 번호: {{repair_number}}' || char(10) || '운송장 번호: {{tracking_number}}' || char(10) || '배송 조회: {{tracking_url}}' || char(10) || char(10) || '완료된 수선 기록과 배송 정보는 아래 Repair Ticket에서도 확인할 수 있습니다.' || char(10) || '{{repair_ticket_url}}' || char(10) || char(10) || 'Studio OALUM을 이용해주셔서 감사합니다.','["customer_name","product_name","repair_number","tracking_number","tracking_url","repair_ticket_url"]','["customer_name","tracking_number","repair_ticket_url"]',0,1),
    ('repair.payment_confirmed_shipping_started','sms','repair','입금 확인 및 배송 시작','입금 확인과 배송을 한 문자로 안내합니다.','입금 확인 및 배송','','[Studio OALUM 수선 안내]' || char(10) || '{{customer_name}}님, 입금을 확인했으며 수선 제품을 발송했습니다.' || char(10) || char(10) || '운송장 번호: {{tracking_number}}' || char(10) || '배송 조회: {{tracking_url}}' || char(10) || '수선 기록: {{repair_ticket_url}}' || char(10) || char(10) || '감사합니다.','["customer_name","tracking_number","tracking_url","repair_ticket_url"]','["customer_name","tracking_number","repair_ticket_url"]',2000,1),
    ...
    ('ticket.admin_message_to_customer','email','ticket','관리자 티켓 답변 알림','관리자 답변을 고객에게 알립니다.','관리자 메시지','[Studio OALUM] 새 답변이 도착했습니다 · {{repair_number}}','안녕하세요, {{customer_name}}님.' || char(10) || char(10) || '수선 담당자가 Repair Ticket {{repair_number}}에 새 답변을 남겼습니다.' || char(10) || '현재 상태: {{repair_status}}' || char(10) || char(10) || '아래 Repair Ticket에서 답변을 확인하고 추가 문의를 남기실 수 있습니다.' || char(10) || '{{repair_ticket_url}}' || char(10) || char(10) || '감사합니다.' || char(10) || 'Studio OALUM','["customer_name","repair_number","repair_status","repair_ticket_url"]','["customer_name","repair_status","repair_ticket_url"]',0,1),
    ('ticket.system_message_to_customer','email','ticket','수선 상태 안내','milestone 외 상태 변경 안내입니다.','상태 변경','[Studio OALUM] 수선 상태 안내 · {{repair_number}}','안녕하세요, {{customer_name}}님.' || char(10) || char(10) || 'Repair Ticket {{repair_number}}의 수선 상태가 변경되었습니다.' || char(10) || '현재 상태: {{repair_status}}' || char(10) || char(10) || '변경 내용과 다음 안내는 아래 Repair Ticket에서 확인해주세요.' || char(10) || '{{repair_ticket_url}}' || char(10) || char(10) || '감사합니다.' || char(10) || 'Studio OALUM','["customer_name","repair_number","repair_status","repair_ticket_url"]','["customer_name","repair_status","repair_ticket_url"]',0,1)
)
