...
WITH template_seed(
  ...
  VALUES
    ...
    ('repair.application_submitted','email','repair','수선 접수 완료','수선 신청 직후 고객에게 보내는 상세 안내입니다.','접수 완료','[Studio OALUM] 수선 접수가 완료되었습니다','안녕하세요, {{customer_name}}님.' || char(10) || char(10) || 'Studio OALUM에 {{product_name}} 수선을 신청해주셔서 감사합니다.' || char(10) || '제품 보내실 곳: {{studio_address}}' || char(10) || char(10) || '신청 내용과 이후 안내는 아래 수선 접수 페이지에서 확인해주세요.' || char(10) || '{{repair_ticket_url}}' || char(10) || char(10) || '실제 수선을 시작할 때 수선 티켓 번호가 발급됩니다.','["customer_name","product_name","repair_number","studio_address","repair_url","repair_ticket_url"]','["customer_name","product_name","studio_address","repair_ticket_url"]',0,1),
    ...
    ('repair.application_submitted','sms','repair','수선 접수 완료','국내 고객에게 보내는 수선 접수 문자입니다.','접수 완료','','[Studio OALUM 수선 안내]' || char(10) || '{{customer_name}}님, 수선 신청을 정상적으로 접수했습니다.' || char(10) || '제품 발송 주소와 이후 안내: {{repair_ticket_url}}' || char(10) || '수선을 시작할 때 티켓 번호가 발급됩니다.','["customer_name","repair_number","repair_ticket_url"]','["customer_name","repair_ticket_url"]',2000,1),
