...
WITH template_seed(
  ...
  VALUES
    ...
    ('repair.application_submitted','email','repair','수선 문의 접수','수선 문의 직후 고객에게 보내는 상세 안내입니다.','문의 접수','[Studio OALUM] 수선 문의가 접수되었습니다','안녕하세요, {{customer_name}}님.' || char(10) || char(10) || 'Studio OALUM에 {{product_name}} 수선을 문의해주셔서 감사합니다.' || char(10) || '제품 보내실 곳: {{studio_address}}' || char(10) || char(10) || '신청 내용과 이후 안내는 아래 수선 문의 페이지에서 확인해주세요.' || char(10) || '{{repair_ticket_url}}' || char(10) || char(10) || '실제 수선을 시작할 때 수선 티켓 번호가 발급됩니다.','["customer_name","product_name","repair_number","studio_address","repair_url","repair_ticket_url"]','["customer_name","product_name","studio_address","repair_ticket_url"]',0,1),
    ('repair.application_submitted_admin','email','repair','새 수선 문의 · 관리자','고객이 수선 문의를 완료하면 관리자에게 보내는 필수 알림입니다.','문의 접수','[Repair] 새 수선 문의 · {{customer_name}}','새 수선 문의가 접수되었습니다. 고객 {{customer_name}}, 이메일 {{customer_email}}, 연락처 {{customer_phone}}, 제품 {{product_name}}, 고객 발송지 {{shipping_address}}. 수선 요청: {{repair_request}}. 관리자 확인: {{repair_admin_url}}','["customer_name","customer_email","customer_phone","product_name","repair_request","shipping_address","repair_ticket_url","repair_admin_url"]','["customer_name","customer_email","customer_phone","product_name","repair_request","repair_admin_url"]',0,1),
    ('repair.application_submitted','sms','repair','수선 문의 접수','국내 고객에게 보내는 수선 문의 문자입니다.','문의 접수','','[Studio OALUM 수선 안내]' || char(10) || '{{customer_name}}님, 수선 문의를 정상적으로 접수했습니다.' || char(10) || '제품 발송 주소와 이후 안내: {{repair_ticket_url}}' || char(10) || '수선을 시작할 때 티켓 번호가 발급됩니다.','["customer_name","repair_number","repair_ticket_url"]','["customer_name","repair_ticket_url"]',2000,1),
