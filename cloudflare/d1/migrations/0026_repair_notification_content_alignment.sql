-- Idempotent production reconciliation for databases that received the 0025
-- Repair schema before the final notification copy and variable names.
...
SET area = 'repair',
    ...
    trigger_label = '신청 완료',
    active_subject = '[Repair] 새 수선 신청 {{repair_number}} · {{customer_name}}',
    active_body = '새 수선 신청이 접수되었습니다.' || char(10) || char(10) ||
      ...
      '{{repair_admin_url}}',
    draft_subject = '[Repair] 새 수선 신청 {{repair_number}} · {{customer_name}}',
    draft_body = '새 수선 신청이 접수되었습니다.' || char(10) || char(10) ||
      '티켓 번호: {{repair_number}}' || char(10) ||
      '고객명: {{customer_name}}' || char(10) ||
      '이메일: {{customer_email}}' || char(10) ||
      '연락처: {{customer_phone}}' || char(10) ||
      ...
      '아래 Repair Ticket에서 신청 사진과 상세 내용을 확인하고 고객에게 답변할 수 있습니다.' || char(10) ||
      '{{repair_admin_url}}',
    default_subject = '[Repair] 새 수선 신청 {{repair_number}} · {{customer_name}}',
    default_body = '새 수선 신청이 접수되었습니다.' || char(10) || char(10) ||
      '티켓 번호: {{repair_number}}' || char(10) ||
      '고객명: {{customer_name}}' || char(10) ||
      '이메일: {{customer_email}}' || char(10) ||
      '연락처: {{customer_phone}}' || char(10) ||
      ...
      '아래 Repair Ticket에서 신청 사진과 상세 내용을 확인하고 고객에게 답변할 수 있습니다.' || char(10) ||
      '{{repair_admin_url}}',
    allowed_variables_json = '["customer_name","customer_email","customer_phone","product_name","repair_number","repair_request","shipping_address","repair_ticket_url","repair_admin_url"]',
    required_variables_json = '["customer_name","customer_email","customer_phone","product_name","repair_number","repair_request","repair_admin_url"]',
    is_enabled = 1,
    activated_at = COALESCE(activated_at, '2026-09-09T00:00:00.000Z'),
