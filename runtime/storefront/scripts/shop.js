...
const activeTag = normalizeShopTag(new URLSearchParams(window.location.search).get("tag")) || "all";

function getProductPath(baseName) {
  const encoded = encodeURIComponent(baseName);
  return `./product.html?product=${encoded}`;
}

function getShopPath(tag) {
  return tag && tag !== "all" ? `./shop.html?tag=${encodeURIComponent(tag)}` : "./shop.html";
}
