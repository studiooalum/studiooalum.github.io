...
function lockCartScroll() {
  if (!document.body || !isMobileCartViewport()) return;

  cartLockedScrollY = window.scrollY || window.pageYOffset || 0;
  const scrollbarGap = Math.max(0, window.innerWidth - document.documentElement.clientWidth);

  document.body.style.setProperty("--cart-scroll-lock-top", `${-cartLockedScrollY}px`);
  document.body.style.setProperty("--cart-scroll-lock-scrollbar-gap", `${scrollbarGap}px`);
  document.body.classList.add("cart-scroll-locked");
}

function unlockCartScroll() {
  if (!document.body) return;

  const restoreScrollY = cartLockedScrollY;
  const wasLocked = document.body.classList.contains("cart-scroll-locked");

  document.body.classList.remove("cart-scroll-locked");
  document.body.style.removeProperty("--cart-scroll-lock-top");
  document.body.style.removeProperty("--cart-scroll-lock-scrollbar-gap");
  cartLockedScrollY = 0;
