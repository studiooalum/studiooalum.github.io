...
function normalizeParticipantLimit(value, fallback, { min = 1, max = 100 } = {}) {
  const limit = Number(value);
  if (!Number.isFinite(limit)) return fallback;
  return Math.max(min, Math.min(max, Math.round(limit)));
}

function normalizeWorkshopType(raw, hasExplicitConfig) {
  const type = String(raw?.workshopType || raw?.type || "").trim();
  const legacyTypes = {
  ...
  if (legacyTypes[type]) return legacyTypes[type];

  if (raw?.mode === "daily") {
    return WORKSHOP_TYPES.DAILY;
  }

  if (raw?.mode === "scheduled" || hasExplicitConfig) {
    return WORKSHOP_TYPES.EVENT;
  }
...
export function getWorkshopBookingConfig(workshop = {}) {
  ...
  const workshopType = normalizeWorkshopType(raw, hasExplicitConfig);
  const mode = workshopType === WORKSHOP_TYPES.DAILY ? "daily" : "scheduled";
  const rawPrices = raw.priceTiers && typeof raw.priceTiers === "object"
    ? raw.priceTiers
...
export function getWorkshopScheduleSlots(workshop) {
  ...
  if (bookingConfig.workshopType === WORKSHOP_TYPES.DAILY) {
